...
export default function ReportsPanel() {
  ...
  const totals = useMemo(() => {
    ...
    return {
      ...
      typeAmount: report.by_violation_type.reduce(
        (s, r) => s + r.amount_assessed,
        0,
      ),
      enforcerTickets: report.by_enforcer.reduce(
        (s, r) => s + r.tickets_issued,
        0,
      ),
      enforcerAssessed: report.by_enforcer.reduce(
        (s, r) => s + r.fines_assessed,
        0,
      ),
      enforcerCollected: report.by_enforcer.reduce(
        (s, r) => s + r.collected,
        0,
      ),
      methodAmount: report.financials.by_method.reduce(
        (s, r) => s + r.amount,
        0,
      ),
          ...
          <div className="report-doc">
            ...
            <div className="report-doc-head">
              <div className="report-org">
                Republic of the Philippines · Municipality of San Jose
              </div>
              <div className="report-org">
                Traffic Management Office (SJTMO)
              </div>
              <div className="report-title">
                {report.meta.period === "yearly" ? "Annual" : "Monthly"}{" "}
                Enforcement &amp; Collection Report
              </div>
              <div className="report-period">{report.meta.label}</div>
              <div className="report-meta">
                <span>
                  Coverage: {report.meta.start} to {report.meta.end} (exclusive)
                </span>
                <span>Prepared by: {report.meta.generated_by}</span>
                <span>
                  Generated: {formatDateTime(report.meta.generated_at)}
                </span>
              </div>
            </div>
            ...
            <Section title="I. Executive Summary">
              <div className="report-stat-grid report-summary-cards">
                <Stat
                  ...
                  hint={
                    <>
                      vs {count(report.comparison.previous_tickets)} in{" "}
                      {report.comparison.previous_label}{" "}
                      <Delta
                ...
                <Stat
                  ...
                  hint={
                    <>
                      vs {peso(report.comparison.previous_collected)} in{" "}
                      {report.comparison.previous_label}{" "}
                      <Delta
              ...
              <table className="report-table">
                ...
                <tbody>
                  {[
                    ...
                    [
                      "Payment Submitted",
                      report.summary.payment_submitted,
                      "Motorist uploaded a receipt, awaiting verification",
                    ],
                    [
                      "Partially Paid",
                      report.summary.partially_paid,
                    ...
                    [
                      "Disputed",
                      report.summary.disputed,
                      "Contested by the motorist",
                    ],
                    [
                      "Dismissed",
                      report.summary.dismissed,
            ...
            <Section title="III. Financial Summary">
              <table className="report-table">
                <tbody>
                  ...
                  <tr>
                    <td>
                      Total collected (verified payments received this period)
                    </td>
                    <td className="num">
                      {peso(report.financials.total_collected)}
                    </td>
                  ...
                  <tr>
                    ...
                    <td className="num">
                      {count(report.financials.payment_count)}
                    </td>
                  </tr>
                  <tr>
                    <td>Tickets receiving a payment</td>
                    <td className="num">
                      {count(report.financials.tickets_paid_against)}
                    </td>
                  </tr>
                  <tr>
                    <td>Motorist-submitted (online) payments verified</td>
                    <td className="num">
                      {count(report.financials.online_submissions)}
                    </td>
                  </tr>
                  <tr>
                    <td>Receipts still awaiting verification</td>
                    <td className="num">
                      {count(report.financials.unverified_count)} (
                      {peso(report.financials.unverified_amount)})
                    </td>
                  </tr>
                </tbody>
              </table>
                  ...
                  {report.by_enforcer.length === 0 ? (
                    <tr>
                      <td colSpan={7} className="report-empty">
                        No tickets issued in this period.
                      </td>
                    </tr>
                  ) : (
            ...
            <Section title="VII. System Activity">
              <div className="report-stat-grid report-summary-cards">
                ...
                />
                <Stat
                  value={count(report.new_users.motorist)}
                  label="New Motorist Accounts"
                />
                <Stat
                  value={count(report.new_users.enforcer)}
                  label="New Enforcer Accounts"
                />
                <Stat
                  value={count(report.new_users.admin)}
            ...
            <div className="report-footnote">
              Notes: Fines assessed are computed from the current fine schedule
              for every violation cited on tickets issued within the coverage
              period. Collections are recognised on the payment date, so a
              payment settling an earlier period&apos;s ticket is reported here
              and the collection rate may exceed 100%. Only verified payments
              are counted; receipts awaiting verification are listed separately
              in Section III. Deleted tickets are excluded.
            </div>
          </div>
        </div>
      )}
