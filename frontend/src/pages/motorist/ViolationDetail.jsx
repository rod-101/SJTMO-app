  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        {/* Status Banner */}
        <div style={{
          background: v.status === 'resolved' ? '#e8f5e9' : v.status === 'dismissed' ? '#f5f5f5' : '#fff3e0',
          ...
          <span style={{ fontWeight: 700, color: statusColor, textTransform: 'capitalize' }}>
            {v.status === 'pending' ? '⚠️ Pending' : v.status === 'resolved' ? '✅ Resolved' : '🚫 Dismissed'}
          </span>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 16 }}>
          ...
          <DetailRow label="Date" value={new Date(v.date_issued).toLocaleDateString()} />
        ...
        {/* Mini Map */}
        {hasLocation && (
          <div style={{ borderRadius: 10, overflow: 'hidden', marginBottom: 16, border: '1px solid #e0e0e0' }}>
            <MapContainer
