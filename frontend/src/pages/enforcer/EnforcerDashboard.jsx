  ...
  return (
    <div className="page">
      ...
      <div className="content enforcer-page">
        ...
        {tab === 'issue' && (
          <IssueViolation onSuccess={() => { setTab('history'); fetchMyViolations(); }} />
        )}

        {tab === 'history' && (
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
              <span style={{ fontSize: '0.85rem', color: '#666' }}>Violations issued by you</span>
              <button className="btn btn-outline btn-sm" onClick={fetchMyViolations}>🔄 Refresh</button>
            </div>
            {loading ? (
              ...
              violations.map((v) => (
                <div key={v.id} className={`violation-card ${v.status}`}>
                  ...
                  {v.notes && (
                    <div style={{ marginTop: 6, fontSize: '0.8rem', color: '#666', fontStyle: 'italic' }}>
                      {v.notes}
                    </div>
                  )}
                </div>
