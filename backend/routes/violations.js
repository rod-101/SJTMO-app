...
router.get('/', async (req, res) => {
  ...
  try {
    ...
    if (motorist) {
      query = 'SELECT * FROM violations WHERE LOWER(motorist_name) = LOWER($1) ORDER BY date_issued DESC';
      params = [motorist];
    }
...
router.post('/', async (req, res) => {
  const { motorist_name, violation_type, notes, latitude, longitude, enforcer_name } = req.body;

  if (!motorist_name || !violation_type || !enforcer_name) {
    return res.status(400).json({ error: 'motorist_name, violation_type, and enforcer_name are required' });
  }
  ...
  try {
    const result = await pool.query(
      ...
      [motorist_name, violation_type, notes || '', latitude || null, longitude || null, enforcer_name]
    );
...
router.put('/:id', async (req, res) => {
  ...
  try {
    const result = await pool.query(
      ...
      [motorist_name || null, violation_type || null, notes ?? null, status || null, enforcer_name || null, id]
    );
    if (result.rows.length === 0) {
      return res.status(404).json({ error: 'Violation not found' });
    }
