...
router.get(
  "/",
  requireAuth,
  authorize("admin", "enforcer", "motorist"),
  async (req, res) => {
    ...
    try {
      ...
      if (status === "paid") {
        const paymentState = await pool.query(
          `SELECT COALESCE((SELECT SUM(p.amount_paid) FROM payments p WHERE p.ticket_id = t.id AND p.verified = TRUE), 0) AS amount_paid,
                COALESCE((SELECT SUM(vt.fine)
                            FROM unnest(string_to_array(t.violation_type, ',')) AS names(n)
                            JOIN violation_types vt ON vt.name = trim(names.n)), 0) AS fine_total
           FROM tickets t
          WHERE t.id = $1`,
          [id],
        );
        const row = paymentState.rows[0];
        if (
          !row ||
          Math.round(Number(row.amount_paid) * 100) <
            Math.round(Number(row.fine_total) * 100)
        ) {
          return res
            .status(400)
            .json({
              error:
                "A ticket can be marked paid only after the full payment is verified.",
            });
        }
      }
...
router.get("/lookup/:token", async (req, res) => {
  try {
    ...
    if (names.length > 0) {
      ...
      const fineByName = new Map(
        finesResult.rows.map((r) => [r.name, Number(r.fine) || 0]),
      );
